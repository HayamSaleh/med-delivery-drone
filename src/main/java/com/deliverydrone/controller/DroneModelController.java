...
public class DroneModelController {
  ...
  public ResponseEntity<DroneModelDto> addDroneModel(@RequestBody DroneModelDto droneModel) {
	return new ResponseEntity<>(droneModelService.addDroneModel(droneModel), HttpStatus.CREATED);
  }

  @PutMapping("/{id}")
  public ResponseEntity<DroneModelDto> updateDroneModel(@RequestBody DroneModelDto droneModel, @PathVariable Long id) {

...
@Service
public class DroneServiceModelImpl implements DroneModelService {
  ...
  public DroneModelDto addDroneModel(DroneModelDto droneModelDto) {
    if (droneModelRepository.existsByName(droneModelDto.getName())) {
      DroneModel savedDrone =
          droneModelRepository.save(dozerMapper.map(droneModelDto, DroneModel.class));
      return dozerMapper.map(savedDrone, DroneModelDto.class);
    }
